...
def main():
    ...
    years = [col for col in df.columns if col.isdigit() and 1800 <= int(col) <= 2050]

    # Get population values
    my_values = df[df['country'] == my_country][years].iloc[0].str.replace('M', '').astype(float) * 1_000_000
    other_values = df[df['country'] == other_country][years].iloc[0].str.replace('M', '').astype(float) * 1_000_000
    ...
    plt.xticks([years[i] for i in range(0, len(years), len(years)//8)], rotation=45)
